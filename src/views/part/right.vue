<template>
  <div class="part-right">
    <div :class="(parseInt(Number(Math.random()*10)%2))==0?'part-right1':''">
      <panle title="供水SCADA数据监测">
        <el-tabs v-model="applyData.activeName">
          <el-tab-pane label="供水压力监测" name="first">
            <el-table :data="applyData.pressData" style="width: 100%" height="158" class="table1">
   ...
   <div :class="(parseInt(Number(Math.random()*10)%2))==0?'part-right2':''">
   ...
   <div :class="(parseInt(Number(Math.random()*10)%2))==0?'part-right3':''">
    <panle title="施工热线信息">
      <el-tabs v-model="hotLineRoadWorkData.activeName">
        <el-tab-pane label="热线信息" name="first">
          <el-table
            :data="hotLineRoadWorkData.hotLineData.slice(0, 5)"
            style="width: 100%"
            height="158"
          >
   ...
   <div :class="(parseInt(Number(Math.random()*10)%2))==0?'part-right4':''">
    <panle title="重点用户列表">
      <el-tabs v-model="vipData.activeName">
        <el-tab-pane label="重点用水户监测" name="first">
          <el-table :data="vipData.useWaterData" style="width: 100%" height="184">
        ...
        <el-tab-pane label="重点排水户监测" name="second">
          <el-table :data="vipData.drainWaterData" style="width: 100%" height="184">
...
<script>
import panle from '@/views/components/panle.vue'
const pressData = function() {
    let data = []
    let name = ['和平弄', '东高桥', '329调流阀后', '南风苑', '陶堰阀后']
    let hb = ['', 'up', '', 'down', '']
    let alarm = [false, false, false, true, false]
    for (let i = 0; i < 5; i++) {
        data.push({
            name: name[i],
            time: '14:18:23',
            value: (Math.random() * 0.2 + 0.2).toFixed(3),
            hb: hb[i],
            alarm: alarm[i]
        })
    }
...
const pumpFluxData = function() {
    let data = []
    let name = ['城东6#', '迪荡1#', '袍江一期', '市排一期', '袍江二期']
    let hb = ['', 'up', '', 'down', '']
    let alarm = [false, false, false, false, false]
    let value = [25.5, 180.6, 2015, 1307, 5486]
    for (let i = 0; i < 5; i++) {
        data.push({
            name: name[i],
            time: '14:18:23',
...
const hotLineData = function() {
    let data = []
    let gdh = ['561738', '561737', '561736', '561735', '561734']
    let fylb = ['咨询', '求助', '查询', '求助', '咨询']
    let zrbm = [
        '客户服务中心',
        '供排水抢修中心',
        '客户服务中心',
...
const useWaterData = function() {
    ...
    let zrlj = ['1321', '868', '143', '412', '504']
    let jrlj = ['1061', '708', '118', '371', '467']
    let bylj = ['26565', '17797', '129709', '10329', '12482']
    for (let i = 0; i < 5; i++) {
        data.push({
            name: name[i],
            zrlj: zrlj[i],
...
export default {
    ...
    data() {
        return {
            ...
            drainData: {
                ...
                pumpFluxData: pumpFluxData()
            },
            hotLineRoadWorkData: {
                activeName: 'first',
                hotLineData: hotLineData(),
                roadWorkData: hotLineData()
            },
            vipData: {
                activeName: 'first',
    ...
    methods: {
        sortPressData() {
            var arr = this.applyData.pressData
            var random = []
            var len = arr.length
            for (var i = 0; i < len; i++) {
                var index = Math.floor(Math.random() * (len - i))
                random.push(this.applyData.pressData[index])
                arr.splice(index, 1)
            }
            this.applyData.pressData = random
            //排水
            arr = this.drainData.pumpFluxData
            random = []
            len = arr.length
            for (var i = 0; i < len; i++) {
                var index = Math.floor(Math.random() * (len - i))
                random.push(this.drainData.pumpFluxData[index])
                arr.splice(index, 1)
            }
            this.drainData.pumpFluxData = random
            //热线
            arr = this.hotLineRoadWorkData.hotLineData
            random = []
            len = arr.length
            for (var i = 0; i < len; i++) {
                var index = Math.floor(Math.random() * (len - i))
                random.push(this.hotLineRoadWorkData.hotLineData[index])
                arr.splice(index, 1)
            }
            this.hotLineRoadWorkData.hotLineData = random
            //vip
            arr = this.vipData.useWaterData
            random = []
            len = arr.length
            for (var i = 0; i < len; i++) {
                var index = Math.floor(Math.random() * (len - i))
                random.push(this.vipData.useWaterData[index])
                arr.splice(index, 1)
            }
            this.vipData.useWaterData = random
        },
        freshData() {
            //this.getSdata()
            //this.getEventData()
        ...
        getBiguserData: function() {
            ...
            $.when(
                ...
                API.geStationDetail(
                    Object.assign({}, gs, {
                        Dtypes: ['SJLJ'],
                        StationId: '55cc645842de38543fe277f3',
                        Begin: n_month,
                        End: n_now,
                        Interval: 86400,
                        All: true
                ...
                API.geStationDetail(
                    Object.assign({}, gs, {
                        Dtypes: ['SJLJ'],
                        StationId: '55cc66a842de38543fe27829',
                        Begin: n_month,
                        End: n_now,
                        Interval: 86400,
                        All: true
    ...
    .el-table {
        ...
        .el-table__body-wrapper {
            ...
            .el-table__row {
                ...
                &:nth-child(odd) {
                   /*  background-color: rgb(20, 33, 46) !important; */
                    background-color: rgba(37,45,62,0.4)!important;
                }
                /* &:nth-child(3) {
                       color:white!important;
                    background-color:#f53008!important;
                ...
                td {
                    ...
                    .arrows {
                        ...
                        margin-left: 15px;
                        &.up {
                            background: url("https://lg-m1opkjhk-1256679880.cos.ap-shanghai.myqcloud.com/arrows-up.png");
                        }
                        &.down {
                            background: url("https://lg-m1opkjhk-1256679880.cos.ap-shanghai.myqcloud.com/arrows-down.png");
                        }
                    }
                }
            }
        ...
        .el-table__body-wrapper {
            .el-table__row {
                &:nth-child(3) {
                    ...
                    background-color: rgba(245,48,8,0.5) !important;
                    /* opacity: 0.4 !important; */
                }
            }
        }
...
.part-middle .el-tabs__header .el-tabs__nav-wrap .el-tabs__item.is-active {
    color: #d2d6d9;
...
.part-middle .el-tabs__header .el-tabs__nav-wrap .el-tabs__item {
    color: #6d6d6d;
    height: 22px;
    font-size: 12px;
    line-height: 22px;
